...
async function testGPSTracking() {
    ...
    try {
        ...
        const signupRes = await axios_1.default.post(`${API_BASE_URL}/auth/signup`, {
            name: 'GPS Test Rider',
        ...
        const updateRes = await axios_1.default.post(`${API_BASE_URL}/rider/update-location`, locationUpdate, {
            headers: { Authorization: `Bearer ${riderToken}` },
        });
        if (updateRes.data.success) {
            console.log(`✅ Location updated: ${locationUpdate.latitude}, ${locationUpdate.longitude}`);
            console.log(`   Response: ${JSON.stringify(updateRes.data.location, null, 2)}`);
            testsPassed++;
        }
        else {
        ...
        const historyRes = await axios_1.default.get(`${API_BASE_URL}/rider/location-history?limit=10`, {
            headers: { Authorization: `Bearer ${riderToken}` },
        });
        if (historyRes.data.success && historyRes.data.locations.length > 0) {
            const latestLocation = historyRes.data.locations[0];
            console.log(`✅ Location history retrieved (${historyRes.data.total} records)`);
        ...
        const socket = (0, socket_io_client_1.default)(WS_URL, {
            path: '/tracking',
            reconnection: true,
        ...
        for (const loc of locations) {
            const res = await axios_1.default.post(`${API_BASE_URL}/rider/update-location`, {
            ...
            if (res.data.success) {
                console.log(`✅ Location ${updateCount + 1}: ${loc.latitude.toFixed(6)}, ${loc.longitude.toFixed(6)} (${loc.description})`);
                updateCount++;
            }
        }
        ...
        for (const coord of testCoordinates) {
            ...
            if (res.data.success && res.data.location) {
                ...
                if (latMatches && lonMatches) {
                    ...
                    console.log(`❌ ${coord.name}: Precision loss detected`);
        ...
        if (accuracyTestsPassed === testCoordinates.length) {
            console.log(`✅ All ${accuracyTestsPassed} precision tests passed`);
            testsPassed++;
        }
        else {
            console.log(`❌ Only ${accuracyTestsPassed}/${testCoordinates.length} precision tests passed`);
            testsFailed++;
        }
        console.log('\n💾 Step 7: Verifying database storage...');
        const finalHistoryRes = await axios_1.default.get(`${API_BASE_URL}/rider/location-history?limit=20`, {
            headers: { Authorization: `Bearer ${riderToken}` },
        });
        if (finalHistoryRes.data.success &&
            finalHistoryRes.data.locations.length >= 5) {
            console.log(`✅ Database storage verified (${finalHistoryRes.data.locations.length} records stored)`);
            console.log('\n   Sample stored records:');
            finalHistoryRes.data.locations.slice(0, 3).forEach((loc, idx) => {
                console.log(`   ${idx + 1}. Lat: ${loc.latitude}, Lon: ${loc.longitude} | Accuracy: ${loc.accuracy}m | Time: ${new Date(loc.timestamp).toISOString()}`);
            });
            testsPassed++;
        }
        else {
        ...
        socket.emit('subscribe-tracking', { awb: 'TEST-GPS-001' }, (response) => {
            console.log(`✅ Subscribed to tracking: ${response.message}`);
        });
        const wsLocationRes = await axios_1.default.post(`${API_BASE_URL}/rider/update-location`, {
            latitude: 23.8103,
            longitude: 90.4161712,
            accuracy: 8,
...
testGPSTracking();
//# sourceMappingURL=test-gps-tracking.js.map