...
export class ShipmentsService {
  ...
  async findAll(
    ...
    // Create query builder for search with eager loading
  ...
  async findOne(id: string, user: User): Promise<Shipment> {
    const shipment = await this.shipmentRepository
      ...
      .cache(`shipment_${id}`, 120000) // Cache for 2 minutes
      .getOne();
  ...
  /**
   * Get shipment by AWB (public tracking - with caching)
   */
  async findByAWB(awb: string): Promise<Shipment> {
    const shipment = await this.shipmentRepository
      ...
      .cache(`shipment_awb_${awb}`, 300000) // Cache for 5 minutes (public tracking)
      .getOne();
